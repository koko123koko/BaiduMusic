...
//        imageLoader = ImageLoader.getInstance();
    ...
    protected BTestAdapter.ViewHolder buildHolder(View view, T t, int i) {
        ...
        if (pos == 16) {
            viewHolder.ivSong = bindView(R.id.iv_item_song_list, view);
            viewHolder.numSong = bindView(R.id.tv_item_song_list_num, view);
            viewHolder.authorSong = bindView(R.id.tv_item_song_list_author, view);
            viewHolder.titleSong = bindView(R.id.tv_item_song_list_title, view);
            viewHolder.iBtnSong = bindView(R.id.ibtn_item_song_list, view);
        }
        if (pos == 8) {
            viewHolder.imageViewBg = bindView(R.id.iv_item_base_scene_blue, view);
            viewHolder.imageViewSrc = bindView(R.id.iv_item_base_scene_icon_blue, view);
            viewHolder.tvBg = bindView(R.id.tv_item_base_scene_blue, view);
        }
        if (pos == 10 || pos == 14) {
            viewHolder.circleImage = bindView(R.id.item_base_resong_div_img, view);
            viewHolder.tv = bindView(R.id.tv_item_base_resong_name, view);
            viewHolder.author = bindView(R.id.tv_item_base_resong_author, view);
            viewHolder.imageButton = bindView(R.id.ibtn_item_base_resong, view);
